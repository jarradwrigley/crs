...
import { NewDeviceData, NewSubscriptionData, RegisterData } from "@/types/auth";
...
export default function SubscribeForm({ mode }: AuthFormProps) {
  ...
  const [subscribeResult, setSubscribeResult] = useState<any | null>(null);
  ...
  const { login, register,   addSubscription } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        const registerData: NewSubscriptionData = {
          imei: deviceIMEI,
          plan: selectedPlan,
          files,
        };

        const response = await addSubscription(registerData);
        ...
        setSubscribeResult(response.data);

        setShowRegistrationSuccess(true);
        setRegistrationSuccessData({
          email: response.data.email || "MOCK",
      ...
      // Handle registration success with email verification required
        ...
        mode === "register" &&
        err.code === "REGISTRATION_SUCCESS_VERIFICATION_REQUIRED"
      ) {
        setShowRegistrationSuccess(true);
        setRegistrationSuccessData({
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 mx-4">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                Device Encryption Initiated!
              </h3>

              {/* Message */}
              <div className="text-gray-600 mb-6 space-y-2">
                <p>
                  {registrationSuccessData?.message ||
                    "Your device is now encrypted."}
                </p>
                {/* <p className="text-sm">
                  Device: <strong>{registrationSuccessData?.email}</strong>
                </p> */}
                <div className="my-4" />
                <p className="text-sm text-gray-500">
                  You can now manage encryption settings from your dashboard.
                </p>
              </div>
